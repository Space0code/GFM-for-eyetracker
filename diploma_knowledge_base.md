...
- A pretrained GazeMAE transfer baseline is now part of the planned comparison set. It freezes pretrained position and velocity GazeMAE encoders, converts each 10-second MAHNOB window into five 2-second chunks at 500 Hz, pools chunk embeddings with mean and standard deviation into a 512-dimensional window representation, and trains only an MLP head. The required inference encoder and converted encoder-only weights are packaged locally in this repository, so experiments do not depend on an external local GazeMAE checkout at runtime. For MAHNOB preprocessing, coordinates are clipped to the actual recording resolution `1280x800` with no scaling or mean normalization. This follows the GazeMAE paper's raw-coordinate principle while avoiding artificial `800 -> 1024` vertical scaling that would distort vertical position amplitudes and derived velocity magnitudes. A detailed code-grounded implementation description for thesis writing is in `docs/appendix/gazemae_mlp_baseline.md`.
- The first full 3-class valence quick comparison including this self-contained `GazeMAE_MLP` baseline completed end-to-end on 2026-05-18 after fixing the quick runner/trainer path so baselines and GNN_v2 use aligned subject folds in one suite invocation. On standard aggregated subject-kfold metrics, balanced accuracy was `GazeMAE_MLP=0.5060`, `GNN_v2=0.5026`, `Random=0.3355`, and `Majority=0.3333`. This makes GazeMAE_MLP a meaningful transfer baseline for the thesis comparison, while still keeping the main model-development narrative focused on the GNN architecture and the cleaner low/high valence target.
