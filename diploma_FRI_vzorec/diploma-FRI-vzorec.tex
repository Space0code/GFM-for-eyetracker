...
Poglavje \ref{ch:struktura} na kratko predstavi tipične sestavne dele diplomske naloge.
V \ref{ch:osnove}.~poglavju spoznamo osnovne gradnike \LaTeX{a} in nekaj koristnih praktičnih nasvetov v zvezi z uporabo \LaTeX{a}. 
V \ref{ch:slo}.~poglavju opozarjamo na nekaj najpogostejših slovničnih napak, ki jih delamo v slovenščini.
\ref{ch:plovke}.~poglavje predstavi vključevanje plovk: slik in tabel.
V \ref{ch:koda}~poglavju spoznamo vključevanje programske kode. 
V \ref{ch:mat}.~poglavju so na hitro predstavljeni besedilni konstrukti kot so izreki, enačbe in dokazi ter kako se na njih sklicujemo.
V \ref{ch:lit}.~poglavju se bomo srečali z iskanjem in sklicevanjem na literaturo,
Na koncu sledi še Zaključek.
...
Diplomska naloga ima natančno določeno zgradbo, ki študenta vodi skozi proces dokumentiranja inženirskega ali raziskovalnega dela. Ustaljena struktura bralcu, predvsem pa mentorju in članom komisije, omogoča hitro navigacijo in preverjanje znanstvene integritete opravljenega dela.

Najpogostejša struktura diplomske naloge na FRI obsega naslednje obvezne in priporočene gradnike:
...
\begin{description}
    ...
    \item[Osrednja poglavja] tvorijo hrbtenico naloge. Pri računalniških diplomah zajemajo sledeča področja, vendar pa jih lahko razbijate na več ločenih poglavij in podpoglavij, lahko jih tudi poimenujete po svoje, odvisno tudi od računalniškega podpodročja v katerem delujete:
    \begin{itemize}
        \item \textbf{Metodologija oz. teoretične osnove:} Razlaga konceptov, potrebnih za razumevanje rešitve.
        \item \textbf{Analiza in načrtovanje:} Opis arhitekture sistema, podatkovnih modelov in uporabniških zahtev.
        \item \textbf{Implementacija:} Opis rešitve, podrobnosti razvoja, uporabljene knjižnice in reševanju specifičnih tehničnih izzivov, vendar brez dejanskih števik.
        \item \textbf{Eksperimenti, evalvacija in rezultati:} Testiranje rešitve, primerjava zmogljivosti ali uporabniška študija, predstavitev rezultatov in argumentacija.
    \end{itemize}
    
    \item[Zaključek] povzame dosežene cilje in poda kritično presojo opravljenega dela. Pomembno je, da izpostavite omejitve svoje rešitve in nakaže smeri za morebitno nadaljnje delo ali izboljšave.
...
Na žalost marsikatero podjetje (in človek) nima dovolj velike ljubezni do mednarodnega sistema enot in boste pogosto videli napisan \qty{1}{\tera\byte}, uporabljen pa je bil pretvornik $1024$ in gre za skoraj \qty{10}{\percent}, oz. \qty{99511627776}{\byte} razliko. Tudi delikatesno naročanje \qty{2}{\hecto\gram} sira namesto uveljavljenih \qty{20}{\deca\gram} kdaj ne bo uspešno, a se, vsaj v diplomi, ne predajte.
...
\begin{itemize}
    ...
    \item \textbf{Tropičje:} V slovenščini je tropičje praviloma nestično ločilo. V \LaTeX u ga zapišemo z ukazom \texttt{\textbackslash dots} in ne s tremi pikami. Primer: \enquote{pes, maček, rosomah, \dots} Izjema je tropičje, ki mu sledi drugo ločilo; takrat je levonestično, npr. \enquote{pes, maček, rosomah, \dots!}
...
\chapter{Vključevanje programske kode}
\label{ch:koda}
...
Ste opazili kako so indukcijski koraki lepo poravnani? Uporabili smo pomemben \texttt{align}. Ste opazili, da smo stavek pri neenakosti~\ref{eq:1} končali s piko? V primeru, da je enačba del povedi, kot npr.
 \begin{equation}
    ...
    \label{eq:1b}
  \end{equation}
pa seveda dodamo vejico v samo enačbo, kot je prikazno pri~\ref{eq:1b}.

Opazimo, da je \LaTeX\ številko izreka podredil številki poglavja.
Na podoben način se lahko s pomočjo ukazov \verb|\label| in \verb|\ref| sklicujemo tudi na druge besedilne konstrukte, kot so med drugim poglavja, podpoglavja in plovke, ki jih bomo spoznali v naslednjem poglavju. Za kakovostno tehnično pisanje pa moramo obvladati še zapis matrik in obdelavo zelo dolgih enačb.
